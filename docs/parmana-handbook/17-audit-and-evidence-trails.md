...
covered in Chapter 8/10's territory, not here), which has the same three checks but is
enforced (on everywhere except `NODE_ENV` `test` and `development`) precisely because a
violation there must refuse a real execution. The class's own comment names this distinction explicitly and traces it to
