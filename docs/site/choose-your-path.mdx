...
| **Run Parmana in production**                                                                    | [Production deployment](/deployment/production)                                                                              | A host that runs Node 24 or later or a container, a Postgres database (Supabase), signing keys | Follow the page top to bottom                                               |
...
If you are an agent integrating Parmana on someone's behalf, do these in order and stop at the first failure:

1. Read the [Full integration overview](/guides/full-integration-overview).
2. Ask the operator for the Parmana API URL and an API key. Do not invent either.
3. Call `GET /health` on that URL and confirm it answers `200` before anything else.
4. Send exactly one test `POST /execute` using a value you generate for `businessTransactionId`.
5. Apply the answer table above without deviation. When in doubt, do not proceed with the action.
