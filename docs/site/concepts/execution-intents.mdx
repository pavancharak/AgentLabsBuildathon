...
<Note>
  ...
  published TypeScript SDK (1.2.0) has none, so use the API route or the script
  above. TypeScript SDK 1.3.0, built but not yet published, adds
  `verifyExecutionIntentOffline`. See [Use the SDKs](#use-the-sdks) and [SDK
  coverage of the API](/sdks/api-coverage).
</Note>
...
- The SDK methods for the intent routes are available from SDK 1.2.0. Earlier versions do not have them. The published TypeScript SDK (1.2.0) has no offline intent verifier; 1.3.0, built but not yet published, adds one.
