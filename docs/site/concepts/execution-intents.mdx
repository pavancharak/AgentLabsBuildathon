...
<Warning>
  ...
  They are in SDK 1.2.0, which is prepared and not yet published. Until then,
  use the HTTP API.
</Warning>
...
- The SDK methods for the intent routes are in the SDK source and are **not in the published 1.1.6**. They are in SDK 1.2.0, which is prepared and not yet published. The TypeScript SDK has no offline intent verifier.
