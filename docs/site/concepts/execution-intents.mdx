...
| The release stage raises an error                                     | `502 EXECUTION_OUTCOME_UNKNOWN` (since 2026-09-25; before, the connector's error, usually a bare `500`). An error that already has its own response, such as `503 CONNECTOR_NOT_REGISTERED`, is returned unchanged. | Intent in `ERRORED`, with `failureReason`.                            | The action may have run. Check the target system, then close the intent with resolve, recording what you found.      |
...
<Warning>
  On `EXECUTION_RECORD_INCOMPLETE` the action **was released**, and on
  `EXECUTION_OUTCOME_UNKNOWN` it **may have been**. In both cases do not
  resubmit it under a new `businessTransactionId`. That could perform the action
  twice.
</Warning>
...
<Note>
  Both SDKs have an offline verifier: `verify_execution_intent_offline` in
  Python, and `verifyExecutionIntentOffline` in TypeScript from 1.3.0 (published
  2026-09-25). On TypeScript 1.2.0 and earlier, use the API route or the script
  above. See [Use the SDKs](#use-the-sdks) and [SDK coverage of the
  API](/sdks/api-coverage).
</Note>

## Use the SDKs

<Warning>
  These methods are available from SDK 1.2.0 (published 2026-09-21). On 1.1.6
  and earlier, use the HTTP API.
</Warning>
...
- The SDK methods for the intent routes are available from SDK 1.2.0. Earlier versions do not have them. The TypeScript SDK has an offline intent verifier from 1.3.0, published 2026-09-25; 1.2.0 has none.
