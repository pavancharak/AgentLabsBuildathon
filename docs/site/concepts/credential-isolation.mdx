...
<Info>
  **[AVAILABLE] as a mechanism, [PARTIAL] as a system-wide guarantee.**
  ...
  below before treating this as true of every connector.
</Info>
...
- **Never in a thrown error.** Both providers' failure messages name only identifiers
  (`connectorId`, environment variable _name_), never a resolved value. Tested in
...
real example (a single token), copied verbatim from current source:
...
exercised for the one connector registered in production, `hubspot`.**
`createHubSpotCredentialProvider.ts` constructs a dedicated `CredentialProvider`
implementation, registered against it (see [The gateway](/concepts/the-gateway)). The
isolation guarantee (caller never sees the credential, single-use, revocable) is
architecturally true for *any* connector routed through this seam, and is proven
generically by the tests cited above, but making it true for a *new* connector today means
writing bootstrap code (a new `CredentialProvider` entry, a new registry registration), not
configuration. There is no dynamic, general-purpose "any connector automatically gets
isolated credentials" path yet. Treat the isolation guarantee as proven for the mechanism
and for `hubspot` specifically, not as a system-wide property of arbitrary connectors you
haven't wired yet.

Two other connectors were previously registered here — `razorpay` (a real,
`createRazorpayCredentialProvider.ts`-backed registration; the connector was deliberately
removed from the repository entirely, 2026-08-12) and `vendor-payment` (a reference
example, also removed entirely, not merely superseded, and `createCredentialProvider.ts`,
the file this warning used to cite, no longer exists). See `docs/CLAIMS.md`'s "Key
Compromise Notice" section and `docs/VERIFICATION-GAPS.md` G-27 in the source repository
