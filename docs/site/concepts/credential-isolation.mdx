...
exercised for the connectors currently registered in production** (`hubspot`, `github`,
`paytm`, `slack`), each with its own dedicated `CredentialProvider` implementation
registered against it (see [The gateway](/concepts/the-gateway)). The isolation guarantee
(caller never sees the credential, single-use, revocable) is architecturally true for *any*
connector routed through this seam, and is proven generically by the tests cited above, but
making it true for a *new* connector today means writing bootstrap code (a new
`CredentialProvider` entry, a new registry registration), not configuration. There is no
dynamic, general-purpose "any connector automatically gets isolated credentials" path yet.
Treat the isolation guarantee as proven for the mechanism and for the connectors already
wired, not as a system-wide property of one you haven't wired yet.

A `razorpay` connector and a `vendor-payment` reference connector were previously registered
here; both were removed from the repository entirely, not merely superseded, see
[Changelog](/changelog).
