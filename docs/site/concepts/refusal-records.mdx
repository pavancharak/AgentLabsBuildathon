...
2. **Only production (Supabase) audit sinks sign.** `SupabaseCallerAuditSink` signs every
   event; the in-memory test sinks (`NODE_ENV=test`) do not. Existing rows written before this capability shipped remain
