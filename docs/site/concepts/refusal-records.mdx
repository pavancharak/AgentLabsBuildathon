...
- **`POST /audit/verify`** closes the analogous gap for caller-authentication audit
  trails — one route, structurally checks and verifies any signed audit event's signature,
  also mounted ahead of caller-auth. See [Error
