...
- **Policy bound and fail closed, signal fresh when opted in.** `policyContentHash` is required at the gateway and checked against the signed approval record, while `signalsHash` is optional. Together they let a
