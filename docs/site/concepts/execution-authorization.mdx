...
covered in depth on [The gateway](/concepts/the-gateway), and two further optional hashes
covering the same "does this still hold at execution time" question for what content
binding doesn't reach: `policyContentHash` (the exact policy *content*, not just its
version string, that produced the decision) and `signalsHash` (the runtime `PolicySignals`
the decision was evaluated against — vendor status, risk exposure, and similar). Both are
optional so authorizations signed before either field existed keep verifying unchanged; see
[The gateway](/concepts/the-gateway) for what each closes and its own scope caveats.
