...
| For each policy approver        | Node.js 24 or newer, a clone of the repository with `npm install`, and OpenSSL 1.1.1 or newer.                            |
...
- **Approvers need the repository.** Signing an approval uses a TypeScript script from the repository, run with Node.js on the approver's machine. A packaged tool does not exist yet.
