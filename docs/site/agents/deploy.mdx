...
6. The maker and the checker in step 8 must be two different identities. You may act as the maker. You must never approve your own proposal. The checker is a human.
...
| `CHECKER_ID`         | The operator. The name of the human checker.                               | It differs from `FIRST_CALLER_ID`.                                            |
...
Replace the three capitalised words with the input values. Each command prints a raw key, and a JSON entry. The second also prints a step up private key.

**Expect:** two JSON entries, each with `callerId` and a `keyHash` of 64 lowercase hexadecimal characters. The second has `credentialHolderType` set to `USER` and a `stepUpPublicKey`.
...
2. Hand the checker bearer key and the step up private key to the checker, and to nobody else.
3. Keep the two JSON entries. Join them into one array. That array is `PARMANA_API_KEYS`.
...
Follow [Integrate Parmana: specification for AI agents](/agents/integrate) with a small, harmless action that the policy allows.

**Expect:** the response is a signed Execution Trust Record. Sending the same `businessTransactionId` again returns `409`.

**If not:** use the table in that page. Never send the same action again under a new id after a `500` with the code `EXECUTION_RECORD_INCOMPLETE`. That action was released. Tell the operator.
