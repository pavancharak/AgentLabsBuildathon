...
<Info>**[AVAILABLE] as a library**, `packages/connector-sdk`, **[AVAILABLE] wired into the default server for HubSpot specifically**, a code change for anything beyond that one today, see the warning below.</Info>
...
if (hubspotCredentialProvider === undefined) {
  ...
  registrations.push({
    ...
    connectorIdentity: { connectorId: "hubspot", publicIdentity: "spiffe://parmana/connectors/hubspot", authenticationMetadata: {} },
...
`docs/CLAIMS.md`'s "Key Compromise Notice" section and the connector-removal history in the
source repository). The lessons below are kept as a historical, worked example — a second
real connector, built after HubSpot's own lessons already existed to apply — not a claim
that Razorpay is still registered anywhere in this codebase today.
</Note>
