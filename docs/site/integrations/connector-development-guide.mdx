...
<Info>
  **[AVAILABLE] as a library**, `packages/connector-sdk`, **[AVAILABLE] wired
  into the default server for HubSpot specifically**, a code change for anything
  beyond that one today, see the warning below.
</Info>
...
variable that adds a connector, this is a code change today. HubSpot is the one connector
actually reachable in the default production server today (GitHub is also registered, for
`github:pr-fetch`/`github:pr-merge`), see [HubSpot](/integrations/hubspot) for that
connector specifically. See
...
<Note>
  Razorpay was deliberately removed from the codebase in full, 2026-08-12 (see
  `docs/CLAIMS.md`'s "Key Compromise Notice" section and the connector-removal
  history in the source repository). The lessons below are kept as a historical,
  worked example — a second real connector, built after HubSpot's own lessons
  already existed to apply — not a claim that Razorpay is still registered
...
[HubSpot](/integrations/hubspot)'s own [FUTURE] section on its page.
