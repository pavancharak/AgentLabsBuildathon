...
<Info>
  **[AVAILABLE]**, hermetic test suite plus a live run against a real HubSpot
  developer/test account. `packages/connector-hubspot/`, `docs/CLAIMS.md` §3.10.
  This was the second connector in this codebase (after Razorpay, since removed
  in full 2026-08-12 — see `docs/CLAIMS.md`'s "Key Compromise Notice" section)
  that talks to a real external system; everything else offered as a reference
  (SAP, Salesforce, Workday, Oracle) is a mock, and no other capability
  registers by default at all, see [The gateway](/concepts/the-gateway).
</Info>
...
<Warning>
  ...
  **empty by default** — no real business-approver key is provisioned in this
  ...
  tested, and unconditionally wired in; it just has nothing to trust yet. See
