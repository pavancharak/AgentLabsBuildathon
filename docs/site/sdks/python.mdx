...
<Info>
  **[AVAILABLE]**, **published**, `parmana` on PyPI. Real, public, installable
  ...
  quickstart script itself runs. v1.1.2 added `create_business_transaction()`
  (see below) and the `offline_verifier.py` fix. Two quick follow-up releases,
  v1.1.3 and v1.1.4, corrected the published package metadata (repo name, docs
  domain, added a `Website` URL); v1.1.4, published 2026-09-14, is the current
  version, confirmed against the live PyPI registry. See [Changelog](/changelog)
