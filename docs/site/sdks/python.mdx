...
<Info>
  **[AVAILABLE]**, **published**, `parmana` on PyPI. Real, public, installable
  ...
  quickstart script itself runs. v1.1.2, adding `create_business_transaction()`
  (see below) and the `offline_verifier.py` fix, published 2026-09-14 —
  confirmed against the live PyPI registry. See [Changelog](/changelog) and the
  fix history at the bottom of this page.
</Info>

<Note>
  v1.1.2's published package metadata and README (`pyproject.toml`'s
  `[project.urls]`, the PyPI page's rendered long description) still point at
  this repository's old name, `parmana-exp`, and the pre-fix docs domain — the
  repo was renamed to `parmana` and the docs domain corrected after that
  version's build artifacts were created, and PyPI package metadata is immutable
  per version. Does not affect installing or using the package; fixed in source
