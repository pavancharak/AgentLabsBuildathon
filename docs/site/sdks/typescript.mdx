...
<Info>
  **[AVAILABLE]**, **published**, [`@parmana/sdk` on
  ...
  v1.2.0, published 2026-09-21 and confirmed via `npm view @parmana/sdk
  version`, is the current version. It adds the Execution Intent methods.
  v1.1.6, published 2026-09-20, builds a default `HttpTransport` when you omit
  ...
  **1.3.0 is built and tested but not yet published**: it adds policy
  governance, offline verification, `caller()`, `publicKey()`, `trustRecords()`,
  `latestReceipt()` and `statusCode` on errors. See [New in
  1.3.0](#new-in-130-built-and-tested-not-yet-published).
</Info>
...
## New in 1.3.0 (built and tested, not yet published)

<Warning>
  Everything in this section is in SDK 1.3.0, which is built and tested in this
  repository but **not yet published to npm**. `npm install @parmana/sdk`
  installs 1.2.0, which does not have it. The full list of what each version
