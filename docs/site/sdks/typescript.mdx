...
- `EXECUTION_RECORD_INCOMPLETE` (500): the action **was** released to the execution system, but its signed record
  ...
  and the operator can rebuild the record without repeating the action. This SDK has no method for that, so the
  operator uses `POST /execution-intents/<businessTransactionId>/finalize` over HTTP.
