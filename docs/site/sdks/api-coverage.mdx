...
<Warning>
  **Version.** Rows marked **1.3.0** are in SDK 1.3.0, which is built and tested
  but **not yet published** to npm or PyPI. `npm install @parmana/sdk` and `pip
  install parmana` install 1.2.0, which does not have them. Every other row
  works in 1.2.0.
</Warning>
