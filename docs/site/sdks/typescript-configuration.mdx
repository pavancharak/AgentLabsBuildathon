...
```typescript
import { ParmanaClient, HttpTransport, type Configuration } from "@parmana/sdk";
...
1. **Always pass `transport`.** In 1.1.3, the current release, the constructor throws `ConfigurationError: Transport is required.` when it is missing, even though the `Configuration` type marks it optional and its comment says a default is used. Tested against the published 1.1.3.
2. **Build the transport from the same configuration object.** `HttpTransport` reads `timeout` and `retryPolicy` from the configuration it is constructed with, not from the client. If you write `new HttpTransport({ endpoint, apiKey })` next to a `retryPolicy` on the client, the transport never sees the retry policy and **nothing is retried**, with no error.
...
| `transport`   | `Transport`          | None, required at runtime in 1.1.3 | The object that sends requests. Use `new HttpTransport(configuration)`.                                                                                                          |
| `userAgent`   | string               | None                               | Declared on the type, but the SDK does not send it in 1.1.3, so it has no effect.                                                                                                |
...
Tested: a server that returns `503` twice and then `200`, and a client with `enabled: true` and `maxAttempts: 3`, made 3 requests and returned success. The same client with a transport built from a partial configuration made 1 request and threw `InternalServerError`.
...
| A missing `endpoint` or `transport`           | `ConfigurationError`     | Thrown by the constructor.                                        |
