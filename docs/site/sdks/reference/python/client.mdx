...
```python
def latest_receipt(business_transaction_id: str) -> Receipt
```
...
```python
def propose_policy_change(name: str, version: str, *,
...
```python
def policy_changes(status: str | None = None) -> list[PolicyChangeForReview]
```
