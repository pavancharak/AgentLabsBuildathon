...
<Info>**[AVAILABLE]**, validated against a real, permanent Fly.io deployment (`parmana-api`). Platform-agnostic by design, built against a bare `docker build` / `docker run`, not a specific PaaS's proprietary build system. Source: `DEPLOYMENT.md` (repo root).</Info>
