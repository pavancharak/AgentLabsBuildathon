...
<Info>
  **[AVAILABLE]**, all three runs below are real output from this session,
  commit `651497a`, Node 24.
</Info>
...
<Warning>
  **Run this twice and the signature will differ both times, even with the same
  ...
  deterministic (CLAIMS.md §5). Only *verification* is deterministic. Don't
  build tooling, caching, or dedup logic that assumes identical input produces
  an identical ML-DSA-65 signature, it won't.
</Warning>
