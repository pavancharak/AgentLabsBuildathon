...
<Info>
  **[AVAILABLE]** for everything below, this is what actually runs today.
  Cloud/KMS/multi-instance/HA deployment is [ROADMAP], see [Roadmap](/roadmap).
</Info>
...
Two more routes render that same spec, exempt from caller authentication for the identical
reason, and neither one keeps its own separate copy, so none of the three can drift from
...
**Exposing all three in production is almost always the right default.** The spec contains no
...
behind a network boundary where even the shape of the API is sensitive), any of the three
...
- **No KMS/HSM.** Both private keys are files on disk, exactly the exposure that produced
  ...
  Setting `KEY_PROVIDER` to one of these fails startup loudly rather than silently running
  as if it were `local`.
...
- **Connectors register only when their own credentials are configured.** HubSpot, GitHub,
  ...
  "No connector registered for capability." See [Add a connector with the Connector
...
<CardGroup cols={2}>
  ...
  <Card title="Roadmap" icon="map" href="/roadmap">
    What's designed but not built yet for production deployment: KMS, HA,
    network enforcement.
  </Card>
</CardGroup>
