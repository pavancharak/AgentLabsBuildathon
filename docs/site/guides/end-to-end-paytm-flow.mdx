...
repo's `npm run migrate` doesn't actually work, apply these directly via a `pg` connection
instead):
...
a capability at all (this codebase's connectors are optional by default, this is deliberate,
not an oversight). A mismatch between the two returns `{"error":"unauthorized"}` (401)
instantly. Redeploy both after setting these.
