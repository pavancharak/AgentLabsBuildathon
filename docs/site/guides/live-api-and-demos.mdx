...
<Info>
  **[AVAILABLE]**. Every endpoint, error, and worked example below was run
  against the live deployment while writing this page, not written from the spec
  alone. Base URL: `https://parmana-api-real.vercel.app`. Repo copy of this
  guide: `LIVE-API-GUIDE.md` (repo root), this page and that file are kept in
  sync.
</Info>
...
deployment — `PAYTM_CONNECTOR_URL`/`PAYTM_CONNECTOR_SHARED_SECRET` point at a real,
separately deployed `parmana-paytm-agent` service, which itself calls Paytm's staging API.
An approved `paytm:refund` request now reaches Paytm's real staging API and returns a real
result, proven live end to end including a full cross-service audit trail correlated by
`businessTransactionId`. Full runbook: [End-to-end: agent → Parmana →
Paytm](/guides/end-to-end-paytm-flow).
...
finding in this codebase (see [What Parmana does not claim](/trust-and-claims/what-we-dont-claim)
...
reaches Policy Engine and gets signed, then fails with a `500`
(`No connector registered for capability '<name>'`) at the dispatch stage, since their
...
for `paytm:refund` as of this correction, not yet for HubSpot/GitHub without adding real
credentials.
...
<CardGroup cols={2}>
  ...
  >
    A slower, more conceptual walkthrough of policy authoring than the reference
    above.
  </Card>
  <Card
