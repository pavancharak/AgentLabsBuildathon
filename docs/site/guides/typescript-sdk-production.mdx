...
**Two kinds of `InternalServerError` need different handling (1.1.3).** Check `error.serverCode`. On
...
(see [Execution Intents](/concepts/execution-intents)). On `EXECUTION_OUTCOME_UNKNOWN` (502) the action was released and the connector call failed, so it may have run: never resubmit it either; the operator checks the target system and closes the intent with `resolveExecutionIntent`. The Execution Intent methods (`executionIntent`, `unfinalizedExecutionIntents`, `finalizeExecutionIntent`, `resolveExecutionIntent`) are available from SDK 1.2.0. On 1.1.6 and earlier the operator uses the HTTP API.
