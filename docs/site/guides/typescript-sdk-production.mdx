...
(see [Execution Intents](/concepts/execution-intents)). The Execution Intent methods (`executionIntent`, `unfinalizedExecutionIntents`, `finalizeExecutionIntent`, `resolveExecutionIntent`) are available from SDK 1.2.0. On 1.1.6 and earlier the operator uses the HTTP API.
