...
(see [Execution Intents](/concepts/execution-intents)). The Execution Intent methods (`executionIntent`, `unfinalizedExecutionIntents`, `finalizeExecutionIntent`, `resolveExecutionIntent`) are in the SDK source and are not in the published 1.1.6, so until the next release the operator uses the HTTP API.
