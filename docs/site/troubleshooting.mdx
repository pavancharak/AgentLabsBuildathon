...
3. Close the intent with what you found, `EXECUTED` or `NOT_EXECUTED`, and a note. This needs a verified human credential and never calls the connector:
   ...
   curl -s -X POST "$PARMANA_API_URL/execution-intents/<businessTransactionId>/resolve" \
     -H "Authorization: Bearer $PARMANA_OPERATOR_KEY" -H "Content-Type: application/json" \
     -d '{"resolution":"NOT_EXECUTED","note":"Agent was unreachable; no refund in Paytm for this order."}'
   ```
