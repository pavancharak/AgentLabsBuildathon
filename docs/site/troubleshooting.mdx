...
- `status.state` is `PREPARED`: the context was not saved, so finalize refuses with `409 EXECUTION_INTENT_RESULT_NOT_RECORDED`. Query `execution_audit_events` for the `businessTransactionId` and check the connector's own record to see what ran, then record the outcome by hand.
...
**Fix:** establish the outcome at the connector, using the `businessTransactionId`, `action` and `target` from `GET /execution-intents/<id>`, and record it by hand. There is currently no operation to close the intent afterwards, so it stays in `GET /execution-intents/unfinalized`. See [Execution Intents](/concepts/execution-intents#find-and-repair-a-released-action-with-no-signed-record).
...
**Fix:** check the connector for the `businessTransactionId` and `target` before doing anything else. Finalize cannot help, because no result was saved.
