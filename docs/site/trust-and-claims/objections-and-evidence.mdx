...
| If someone deleted a row from the audit table, would anything notice? | **Honest limit, real gap, not yet closed.** `ExecutionTrustRecord` executions chain via `previousChainHash` (`ExecutionChainCrypto`), tested. `CallerAuditEvent` rows carry no sequence number or chain hash — a deleted row in the underlying table is currently undetectable by any mechanism this repo has. Closing this properly means a schema migration (a chain-hash column) and a change to what `AuditEventCrypto` signs, not a same-session patch — flagged here rather than quietly built, the same discipline this whole page follows. |
...
- **No schema migration for the `CallerAuditEvent` chain-hash gap** (Domain 3's last row).
  That's a real gap, but closing it changes a production database schema and what a signed
  artifact covers — the same category of decision [Execution
  authorization](/concepts/execution-authorization)'s `grantedCapability` field went through
  deliberately, not something to build silently alongside a documentation pass.
