...
discipline](/trust-and-claims/claims-discipline) for why that ordering matters. Compliance
...
claim](/trust-and-claims/what-we-dont-claim). This page indexes evidence for claims already
...
unrevoked." The rows below answer the real mechanism, not the assumed one.
</Warning>
...
<Note>
This repo has three separate signed/durable record mechanisms, not one — `ExecutionTrustRecord`
...
Rows below name which mechanism actually applies.
</Note>
...
this repository — only a manual script (`latency-test.cjs`) requiring a live deployed key
