...
<Warning>
  ...
  these **fails startup loudly** (`KeyBootstrap.create()` throws, naming the
  value) rather than silently falling back to `FileKeyProvider`. See
...
<Note>
  ...
  exposure KMS/HSM custody is designed to prevent, see [Roadmap](/roadmap), Move
  1.
</Note>
