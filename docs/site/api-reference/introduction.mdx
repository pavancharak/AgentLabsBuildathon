...
description: "22 paths, generated from openapi/openapi.yaml into the sidebar under REST API. This page covers what that spec can't express."
---
...
<Info>
  **[AVAILABLE]**. Every path in the sidebar under **REST API** is generated
  ...
  every push. There is, today, no automated check that every route actually
  mounted in `packages/api/src/app.ts` has a matching spec path; this page's
  list is kept in sync by hand. `GET /documentation` and `GET /reference` are
  deliberately excluded: rendered doc surfaces, not API resources, see [Three
  views of the same spec](#three-views-of-the-same-spec) below.
</Info>
