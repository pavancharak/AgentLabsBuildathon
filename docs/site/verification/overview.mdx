...
<Info>
  **[AVAILABLE]**, precisely scoped.
  `packages/runtime/src/services/verification-service.ts`. CLAIMS.md 2.15.
</Info>

<Warning>
  An earlier six-stage verification pipeline (`@parmana/verification`) existed
  and was **deleted** ("retired in Session 5, it had no real implementation and
  no real test coverage," per CLAIMS.md §4). It is not documented here as
  existing. If you see references to Authority/Intent/Evidence verification
  stages elsewhere, they describe that retired package or a not-yet-built future
