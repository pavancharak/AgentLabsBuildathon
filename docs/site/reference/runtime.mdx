...
<Note>
  ...
  recording are configured for that instance. Each is an intentionally optional,
  capability-scoped dependency, and omitting one is valid, deliberate
  configuration, not a bug. Before this, an operator reading logs had no way to
  tell which protections a given deployment was actually running with.
  Construction-time only, not per-request: the configuration doesn't change per
  transaction. See `docs/CLAIMS.md` §2.30.
</Note>
