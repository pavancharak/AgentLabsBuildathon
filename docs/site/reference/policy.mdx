...
| Export | Purpose |
|---|---|
| `PolicyValidator` | `.validate(policy)` checks a `Policy` document's structural integrity (`policyId`/`policyVersion`/`schemaVersion` present, `rules` well-formed, `boundSignals` shape, operators supported), invoked by `PolicyRouter.load()`. `.findUncoveredFacts(policy)` returns every fact a rule condition references that has no `boundSignals` entry — advisory, not thrown; see the note below |
...
<Note>
**boundSignals coverage warning.** A rule can decide on a fact (e.g. `amount`) that isn't
declared in `boundSignals`, silently forfeiting `SignalIntentBinder`'s protection for that
fact. `PolicyRouter.load()` calls `PolicyValidator.findUncoveredFacts(policy)` after
`.validate()` and logs a `policy_boundSignals_coverage_incomplete` warning (never throws)
naming the uncovered facts. Deliberately advisory: a fact with no genuine Intent-side
equivalent (`vendorVerified`, `riskScore`) is legitimately excluded from `boundSignals` and
would otherwise generate constant false positives. See `docs/CLAIMS.md` §2.30.
</Note>
