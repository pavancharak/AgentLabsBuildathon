...
<Info>
  **[AVAILABLE]**. `packages/api`. Not a library other packages import, it's the
  running server. No public exports (`src/index.ts` is empty).
</Info>
...
| `src/app.ts`                               | Mounts the caller-auth middleware (every route except `/health`), all 15 routes, and the error handler, in order, the error handler must be registered last.                                                                                                                                                                                                                                                                                                                                                                                             |
...
`ExecutionControlComposition.ts` and `ConnectorFactory.ts` (a _different_, class-based
