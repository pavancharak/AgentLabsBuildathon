...
<Info>
  ...
  **[PARTIAL]** for the separate `ExecutionPermit` model described below, real
  and tested, not reachable from `packages/api` or `packages/runtime` today.
</Info>
...
| `MemoryExecutionAuditSink`                                                                                                                 | [AVAILABLE]. In-memory only, records `session.created` / `execution.completed` / `execution.rejected` / `authorization.verified` events. Used for tests (`NODE_ENV=test`); production uses `SupabaseExecutionAuditSink` (`@parmana/storage`) instead — durable, signed, chained per `authorizationId`, and queryable. See [@parmana/storage](/reference/storage). |
...
<Note>
  **Signed caller-capability check.** `DefaultConnectorPolicy.assertAllowed()`
  ...
  authorization](/concepts/execution-authorization)) is present — additionally
