...
<Info>
  **[PARTIAL]**. Real semantic re-evaluation, for one execution at a time, not
  reachable from `POST /replay` or any other route. See
  [Replay](/replay/overview) for the full disambiguation from the unrelated
  `POST /replay` route.
</Info>
...
<Warning>
  **Not reachable from the server.** Confirmed by grep: nothing in
  `packages/api` or `packages/runtime` imports `@parmana/replay`. `POST /replay`
  is an unrelated route, a signature recheck via
  `ExecutionTrustApplication.replay()`, see [REST API
  Introduction](/api-reference/introduction). Don't assume calling the HTTP
  route exercises this package.
</Warning>
