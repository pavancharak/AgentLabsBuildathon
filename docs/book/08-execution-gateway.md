...
**`signalsStillCurrent`** (G-31, the newest check, added the same session as this book) —
only when a `SignalStateVerifier` is supplied *and* the authorization carries a `signalsHash`
...
**`nonce`** — last, always, and only attempted if every prior check passed. `isSoleFailureNonceReplay()`
classifies a failure as "this is specifically a replay, not a forgery" only when every other
check passed and nonce consumption alone failed — this is what lets the HTTP layer (Chapter
12) return a distinguishable `409 NONCE_ALREADY_CONSUMED` instead of a generic `500` for that
one specific case.
