[← Book Index](README.md) · [← Previous: Preface](00-preface.md)
...
CRUD entity. The chain, in the order a request actually moves through it, follows.
...
per its own doc comment. Everything downstream (Chapters 15 through 17) reads from this
object, not from the live `BusinessTransaction` or a database join across smaller tables.
The `schemaVersion`/`signatures` pair is worth noting for how this codebase evolves signed
...
[← Book Index](README.md) · [← Previous: Preface](00-preface.md) · [Next: Chapter 2, Policy Engine and Evaluation →](02-policy-engine-and-evaluation.md)
