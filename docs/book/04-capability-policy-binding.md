...
just by name. `RuntimeEngine` (Chapter 5) runs this check **before** `SignalIntentBinder`,
for the same reason `SignalIntentBinder` runs before `PolicyEngine.evaluate`: checking a
