[← Book Index](README.md) · [← Previous: Chapter 4, Capability/Policy Binding](04-capability-policy-binding.md)
...
every subsequent check reads from. Nothing re-fetches or re-derives it, so nothing
downstream can observe a different value than what the policy actually evaluated against.
...
explicitly, not just a dependency ordering. "A policy denial makes zero calls to the
external system" is preserved for every `REJECT` that isn't itself a signal-state mismatch,
by skipping the fetch entirely when the provisional decision was already going to fail. A
...
newest field in the payload (G-31, added the same session this book was written), and
Chapter 7 covers the full envelope shape and why each field exists.
...
[← Book Index](README.md) · [← Previous: Chapter 4, Capability/Policy Binding](04-capability-policy-binding.md) · [Next: Chapter 6, Cryptography →](06-cryptography.md)
