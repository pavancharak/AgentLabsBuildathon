...
validation against a real ₹10 payment with a genuine webhook and signed settlement
...
nonce last) isn't an arbitrary style choice, it's a direct response to a named replay-poisoning
risk; the sign-before-write ordering in Chapter 14 isn't a preference, it's proven by a test
that injects failure at each step specifically because the alternative ordering was
considered and rejected as unsafe. Understanding *why* the code is shaped the way it is —
which is the whole premise of this book — requires knowing what it used to look like, and
what went wrong the one time it didn't look like this.
