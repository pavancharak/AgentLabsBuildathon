...
| Inventory the approvals of the production policies   | Nobody has written down which production policies have human approvals and which (if any) have system approvals from `scripts/backfill-legacy-policy-approvals.ts`, which records no human decision. Answering it needs read access to the production database.     | Query `policy_change_approval_records` in production and record the result.                                                                                     |
...
1. **No Fly.io deployment has been made since the Dockerfile was fixed.** CI proves the image builds and boots, not that Fly runs it.
2. **The success path of the first real request (runbook step 9) has not been exercised end to end.** With no connector configured, the request passes policy and approval and stops at release with `503 CONNECTOR_NOT_REGISTERED`. Proving the full path needs a configured connector, or a local HTTPS mock of the refund service. Outside tests the connector URL must start with `https://`.
