...
- The TypeScript and Python SDKs have methods for the intent routes (G-55, closed in the source on 2026-09-21). **They are not in the published 1.1.6.** They are in SDK 1.2.0, which is prepared and not yet published. The Python SDK has an offline intent verifier that needs only the public key. The TypeScript SDK has none.
