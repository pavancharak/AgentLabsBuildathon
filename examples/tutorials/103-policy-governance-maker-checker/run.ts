...
import { AuthorityType, type PolicyChangeStepUpAuthorization } from "@parmana/shared";
...
const { PolicyChangeStepUpVerifier } = await import(
  "../../../packages/api/src/auth/PolicyChangeStepUpVerifier.js"
);
...
function policyBody(policyId: string) {
  return {
    ...
    schemaVersion: "1.0.0",
    rules: [
      { id: "always-approve", condition: { always: true }, outcome: { action: "approve", reason: "tutorial fixture" } },
    ],
  };
}
...
const scratchPolicyDir = mkdtempSync(path.join(tmpdir(), "parmana-tutorial-103-policies-"));
...
const authenticator = new StaticKeyAuthenticator([
  { callerId: "maker", keyHash: hashApiKey(MAKER_KEY), credentialHolderType: AuthorityType.USER },
  {
    callerId: "checker",
    keyHash: hashApiKey(CHECKER_KEY),
    credentialHolderType: AuthorityType.USER,
    stepUpPublicKey: checkerStepUpKeyPair.publicKey.export({ format: "pem", type: "spki" }).toString(),
  },
  { callerId: "service-caller", keyHash: hashApiKey(SERVICE_KEY), credentialHolderType: AuthorityType.SERVICE },
]);
...
const app = createApp(application, {
  ...
  stepUpVerifier: new PolicyChangeStepUpVerifier({ nonceStore: new MemoryNonceStore() }),
...
try {
  ...
  console.log("--------------------------------------------------");
  const proposeResponse = await fetch(`${baseUrl}/policies/${POLICY_NAME}/1.0.0/pending-changes`, {
    method: "POST",
    headers: { "Content-Type": "application/json", Authorization: `Bearer ${MAKER_KEY}` },
    body: JSON.stringify({ proposedContent: policyBody(POLICY_NAME), reason: "tutorial proposal" }),
  });
  ...
  console.log("Scenario 2: A SERVICE-credentialed caller cannot propose at all");
  console.log("--------------------------------------------------");
  const serviceProposeResponse = await fetch(`${baseUrl}/policies/${POLICY_NAME}-service-attempt/1.0.0/pending-changes`, {
    method: "POST",
    headers: { "Content-Type": "application/json", Authorization: `Bearer ${SERVICE_KEY}` },
    body: JSON.stringify({ proposedContent: policyBody(`${POLICY_NAME}-service-attempt`), reason: "should be denied" }),
  });
  const serviceProposeBody = await serviceProposeResponse.json();
  console.log(`Status : ${serviceProposeResponse.status}, code: ${serviceProposeBody.code}`);
  console.log();

  console.log("Scenario 3: The maker cannot approve its own proposal (maker != checker)");
  console.log("--------------------------------------------------");
  const selfApproveResponse = await fetch(`${baseUrl}/policies/pending-changes/${pendingPolicyChangeId}/approve`, {
    method: "POST",
    headers: { "Content-Type": "application/json", Authorization: `Bearer ${MAKER_KEY}` },
    body: JSON.stringify({}),
  });
  const selfApproveBody = await selfApproveResponse.json();
  console.log(`Status : ${selfApproveResponse.status}, code: ${selfApproveBody.code}`);
  console.log();

  console.log("Scenario 4: A distinct checker, but with NO step-up envelope, is still denied");
  console.log("--------------------------------------------------");
  const noStepUpResponse = await fetch(`${baseUrl}/policies/pending-changes/${pendingPolicyChangeId}/approve`, {
    method: "POST",
    headers: { "Content-Type": "application/json", Authorization: `Bearer ${CHECKER_KEY}` },
    body: JSON.stringify({}),
  });
  const noStepUpBody = await noStepUpResponse.json();
  console.log(`Status : ${noStepUpResponse.status}, code: ${noStepUpBody.code}`);
  console.log();

  console.log("Scenario 5: A distinct checker WITH a valid step-up envelope approves -- file written, signed record persisted");
  console.log("--------------------------------------------------");
  const stepUpAuthorization = await signStepUp(pendingPolicyChangeId, "approve");
  const approveResponse = await fetch(`${baseUrl}/policies/pending-changes/${pendingPolicyChangeId}/approve`, {
    method: "POST",
    headers: { "Content-Type": "application/json", Authorization: `Bearer ${CHECKER_KEY}` },
  ...
  console.log(`Resolved status : ${approveBody.status}, resolvedBy: ${approveBody.resolvedBy}`);

  const writtenPolicy = JSON.parse(
    readFileSync(path.join(scratchPolicyDir, POLICY_NAME, "1.0.0", "policy.json"), "utf8"),
  );
  const records = await policyChangeApprovalRecordRepository.findMostRecentFor(POLICY_NAME, "1.0.0");
  console.log(`Live policy.json written : policyId=${writtenPolicy.policyId}`);
  console.log(`Signed approval record persisted : approvedBy=${records?.approvedBy}, hasSignature=${records?.signature !== undefined}`);
  console.log();
  ...
  if (allPassed) {
    ...
    console.log("✗ Expected every governance layer above to behave exactly as documented.");
  }
