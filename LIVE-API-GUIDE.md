...
*means* conceptually, see `docs/site/` (published docs); for what's cryptographically
...
```jsonc
{
  ...
  "rules": [
    ...
    },
    {
      ...
      "condition": { "fact": "receiptAttached", "operator": "eq", "value": false },
      "outcome": { "action": "reject", "reason": "Expense rejected: no receipt attached." }
    },
    {
      "id": "reject-excessive-amount",
      "condition": { "fact": "expenseAmount", "operator": "gt", "value": 5000 },
      "outcome": { "action": "reject", "reason": "Expense rejected: amount exceeds the $5,000 auto-approval limit." }
    },
    {
      "id": "reject-default",
      "condition": { "always": true },
      "outcome": { "action": "reject", "reason": "Expense rejected: one or more required conditions were not satisfied." }
    }
  ]
}
```
...
```json
{
  "intent": { "action": "expense-reimbursement", "target": "expense-4471", "parameters": { "amount": 1200 } },
  "policy": { "name": "expense-reimbursement", "version": "1.0.0", "schemaVersion": "1.0.0" },
  "signals": { "employeeVerified": true, "expenseAmount": 1200, "receiptAttached": true, "categoryApproved": true }
}
```
